...
class Q7{
  ...
  for(int[] row1:arr1){
    ...
    if(m == n1){   // ✅ Condition check
      int [][]product = new int[n][m1];

      for(int i=0; i<n; i++){         // rows of first
        for(int j=0; j<m1; j++){      // columns of second
          product[i][j] = 0;
          for(int k=0; k<m; k++){     // columns of first / rows of second
      ...
      for(int[] rowProd : product){
        for(int elementProd : rowProd){
          System.out.print(" " + elementProd);
        }
        System.out.println();
      }
